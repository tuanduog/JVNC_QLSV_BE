...
public class AuthService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String tendn) throws UsernameNotFoundException {
        ...
        if(sv != null){
            return new User(sv.getMasv(), sv.getMatkhau(), List.of(new SimpleGrantedAuthority(sv.getQuyen_nd())));
        }

        GiangVien gv = giangVienRepo.findByMagv(tendn).orElse(null);
        if(gv != null){
            return new User(gv.getMagv(), gv.getMatkhau(), List.of(new SimpleGrantedAuthority(gv.getQuyen_nd())));
        }
